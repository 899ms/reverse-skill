...
- **Kali tool discovery: `yara`** — `yara` is in the Kali bootstrap manifest and the Windows tool catalog and is referenced by four skills, but was missing from `kali/scripts/lib/tool-discovery.sh`, so `refresh-tool-index.sh` never reported its path on Kali. Added the catalog entry (`malware-analysis`), matching its sibling analysis CLIs (`binwalk`, `bkcrack`).
- **binwalk could never auto-install on Windows** — `skills/scripts/bootstrap-manifest.json` declared `"bootstrapKind": "winget-package"` with `"wingetId": "ReFirmLabs.binwalk"`, but that id does not exist in the winget community repository (`winget show --id ReFirmLabs.binwalk --exact` → `0x8A150014 NO_APPLICATIONS_FOUND`), and binwalk ships no prebuilt binaries on any GitHub release (`assets: 0` for v3.1.0 and the seven preceding releases), so `bootstrap-reverse.ps1 -Capability binwalk` could only ever fail with a package-not-found error. The Windows entry is now `"bootstrapKind": "manual"` + `"canAutoInstall": false` with a `manualInstallHint` pointing at the Rust toolchain (`winget install Rustlang.Rustup` → `cargo install binwalk`), matching what `firmware-pentest/SKILL.md` already documents. Linux / Kali / macOS are unaffected: `bootstrap-reverse.sh` installs binwalk through apt/brew and does not read this entry. `tool-index.md.template` updated to match.
